...
class TMDbClient:
    ...
    def enrich_movie_data(self, movie_data: Dict) -> Dict:
        ...
        # Genres
        if not enriched_data.get("genres") or enriched_data["genres"].lower() in ["", "unknown genres"]:
            if details.get("genres"):
                genres = [g["name"] for g in details["genres"]]
                enriched_data["genres"] = ", ".join(genres)
        
        # Director
        if not enriched_data.get("director") or enriched_data["director"].lower() in ["", "unknown director"]:
        ...
        if not enriched_data.get("poster_url"):
            if images and images.get("posters"):
                ...
                if poster_path:
                    enriched_data["poster_url"] = f"https://image.tmdb.org/t/p/w500{poster_path}"
    ...
    def batch_enrich_movies(self, movies: List[Dict], delay: float = 0.25) -> List[Dict]:
    ...
    def get_top_rated_movies(self, page: int = 1) -> Optional[Dict]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/movie/top_rated", params={"page": page})
            response.raise_for_status()
