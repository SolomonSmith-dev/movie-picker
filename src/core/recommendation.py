...
from .models import Movie, WatchHistory, UserRating, User
...
class RecommendationEngine:
    ...
    def get_user_preferences(self, user_id: int = 1) -> Dict:
        ...
        with db_manager.get_session() as session:
            ...
            high_rated = session.query(UserRating).filter(
                and_(
            ...
            # Get favorite movies
            favorites = session.query(WatchHistory).filter(
                and_(
                    WatchHistory.user_id == user_id,
                    WatchHistory.is_favorite == True
                )
            ).all()
            ...
            if genres:
                ...
                preferences['favorite_genres'] = [genre for genre, count in genre_counter.most_common(5)]
            
            if directors:
                director_counter = Counter(directors)
                preferences['favorite_directors'] = [director for director, count in director_counter.most_common(3)]
            
            if actors:
                actor_counter = Counter(actors)
                preferences['favorite_actors'] = [actor for actor, count in actor_counter.most_common(5)]
            
            if years:
                # Get preferred year range (most common years ± 5 years)
                year_counter = Counter(years)
                most_common_year = year_counter.most_common(1)[0][0]
                preferences['preferred_years'] = list(range(most_common_year - 5, most_common_year + 6))
            
            # Get minimum rating preference
            if high_rated:
                avg_rating = sum(r.rating for r in high_rated) / len(high_rated)
                preferences['min_rating'] = max(0, avg_rating - 2)  # Suggest movies rated within 2 points of average
    ...
    def get_recommendations(
        ...
        with db_manager.get_session() as session:
            ...
            recommendations = query.order_by(Movie.rating.desc().nullslast()).limit(limit * 2).all()
            
            # If we don't have enough recommendations, add some random ones
            if len(recommendations) < limit:
                remaining = limit - len(recommendations)
                additional = self._get_random_movies(session, remaining, exclude_watched, user_id)
    ...
    def get_similar_movies(self, movie_id: int, limit: int = 5) -> List[Movie]:
        ...
        with db_manager.get_session() as session:
            ...
            if target_movie.director:
                director_movies = session.query(Movie).filter(
                    and_(
            ...
            if target_movie.genres:
                ...
                for genre in genre_list[:2]:  # Top 2 genres
                    genre_movies = session.query(Movie).filter(
                        and_(
            ...
            if target_movie.year:
                ...
                year_movies = session.query(Movie).filter(
                    and_(
                        Movie.year.in_(year_range),
                        Movie.id != movie_id
                    )
                ).limit(limit).all()
