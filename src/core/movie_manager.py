...
class MovieManager:
    ...
    def get_top_rated_movies(self, limit: int = 10) -> List[Movie]:
        ...
        return movie_filter.filter_movies(
            rating_min=7.0, order_by="rating", order_direction="desc", limit=limit
        )
    ...
    def mark_as_watched(
        self, movie_id: int, user_id: int = 1, rating: Optional[float] = None
    ) -> bool:
            ...
            with db_manager.get_session() as session:
                ...
                if existing:
                    ...
                    watch_history = WatchHistory(
                        movie_id=movie_id, user_id=user_id, watched_at=datetime.now()
                    )
    ...
    def get_recommendations(
        self, user_id: int = 1, limit: int = 10
    ) -> Dict[str, List[Movie]]:
