...
class MovieFilter:
        ...
        with db_manager.get_session() as session:
            ...
            if runtime_min is not None or runtime_max is not None:
                if runtime_min is not None and runtime_max is not None:
                    query = query.filter(
                        Movie.runtime.between(runtime_min, runtime_max)
                    )
    ...
    def get_filter_stats(self) -> Dict:
        ...
        with db_manager.get_session() as session:
            ...
            # Available genres
            all_genres = (
                session.query(Movie.genres).filter(Movie.genres.isnot(None)).all()
            )
            genre_set = set()
            ...
            # Available languages
            languages = (
                session.query(Movie.language)
                .filter(Movie.language.isnot(None))
                .distinct()
                .all()
            )
            stats["available_languages"] = [
                lang.language for lang in languages if lang.language
            ]
