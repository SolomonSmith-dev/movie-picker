...
def import_initial_data():
    ...
    try:
        ...
        standardized_movies = movie_manager.load_movies_from_json(
...
def create_default_user():
    ...
    try:
        with db_manager.get_session() as session:
            ...
            existing_user = (
                session.query(User).filter(User.username == "default").first()
            )
            if existing_user:
            ...
            with open(history_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        parts = line.split(" | ")
                        if len(parts) >= 3:
                            _ = parts[0]  # date_str (unused in new format)
                            title_year = parts[1]
                            _ = parts[2]  # director (unused in new format)
                            ...
                            movie = (
                                session.query(Movie)
                                .filter(Movie.title == title, Movie.year == year)
                                .first()
                            )
