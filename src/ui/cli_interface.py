...
class CLIInterface:
    ...
    def display_movie(self, movie: Movie, show_details: bool = False):
        ...
        print(f"⏱️  Runtime: {movie.runtime} min" if movie.runtime else "⏱️  Runtime: N/A")
    ...
    def display_movie_list(self, movies: List[Movie], title: str = "Movies"):
        ...
        for i, movie in enumerate(movies, 1):
            print(f"{i:2d}. {movie.title} ({movie.year}) - {movie.rating}/10" if movie.rating else f"{i:2d}. {movie.title} ({movie.year}) - N/A")
            if movie.director:
    ...
    def filter_by_year(self):
        ...
        try:
            ...
            self.display_movie_list(movies, f"Movies from {year_min or 'any'} to {year_max or 'any'}")
    ...
    def filter_by_rating(self):
        ...
        try:
            ...
            self.display_movie_list(movies, f"Movies rated {rating_min or 'any'} to {rating_max or 'any'}")
    ...
    def filter_by_runtime(self):
        ...
        try:
            ...
            self.display_movie_list(movies, f"Movies {runtime_min or 'any'} to {runtime_max or 'any'} minutes")
    ...
    def multi_criteria_filter(self):
        ...
        try:
            ...
            # Exclude watched
            exclude_watched = input("Exclude watched movies? (y/n): ").strip().lower() == 'y'
    ...
    def show_filter_stats(self):
        ...
        if 'rating_range' in stats:
            print(f"Rating Range: {stats['rating_range']['min']:.1f} - {stats['rating_range']['max']:.1f}")
            print(f"Average Rating: {stats['rating_range']['average']:.1f}")
        
        if 'runtime_range' in stats:
            print(f"Runtime Range: {stats['runtime_range']['min']} - {stats['runtime_range']['max']} minutes")
        ...
        if 'available_languages' in stats:
            print(f"Available Languages: {', '.join(stats['available_languages'][:10])}")
    ...
    def show_movie_of_the_day(self):
        ...
        if movie:
            ...
            if choice == 'y':
                rating = input("Rate this movie (1-10, or press Enter to skip): ").strip()
                try:
    ...
    def show_statistics(self):
        ...
        history = movie_manager.get_watch_history(limit=1000)  # Get all history for stats
        
        if history:
            total_watched = len(history)
            avg_rating = sum(entry['rating'] for entry in history if entry['rating']) / len([entry for entry in history if entry['rating']])
            ...
            if genre_counts:
                ...
                print(f"Top Genres: {', '.join(f'{genre} ({count})' for genre, count in top_genres)}")
        else:
