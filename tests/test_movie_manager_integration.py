...
from src.core.models import Movie, WatchHistory, UserRating
...
class TestMovieManagerIntegration:
    ...
    def test_get_movie_of_the_day_with_data(
        self, populated_test_db, movie_manager_with_test_db
    ):
    ...
    def test_search_movies_with_data(
        self, populated_test_db, movie_manager_with_test_db
    ):
    ...
    def test_search_movies_partial_match(
        self, populated_test_db, movie_manager_with_test_db
    ):
    ...
    def test_get_movies_by_genre(self, populated_test_db, movie_manager_with_test_db):
        ...
        # Get Action movies
        action_movies = movie_manager_with_test_db.get_movies_by_genre(
            "Action", limit=10
        )
    ...
    def test_mark_as_watched_new_movie(
        self, populated_test_db, movie_manager_with_test_db, sample_user, sample_movies
    ):
        ...
        result = movie_manager_with_test_db.mark_as_watched(
            movie_id, user_id, rating=8.5
        )
        ...
        watch_history = (
            populated_test_db.query(WatchHistory)
        ...
        user_rating = (
            populated_test_db.query(UserRating)
            .filter(UserRating.user_id == user_id, UserRating.movie_id == movie_id)
            .first()
        )
    ...
    def test_mark_as_watched_existing_movie(
        self, populated_test_db, movie_manager_with_test_db, sample_user, sample_movies
    ):
    ...
    def test_get_watch_history(
        self, populated_test_db, movie_manager_with_test_db, sample_user
    ):
    ...
    def test_get_watch_history_empty_user(
        self, populated_test_db, movie_manager_with_test_db
    ):
        """Test getting watch history for user with no history."""
        non_existent_user_id = 999

        history = movie_manager_with_test_db.get_watch_history(
            non_existent_user_id, limit=10
        )
    ...
    def test_get_recommendations(
        ...
        recommendations = movie_manager_with_test_db.get_recommendations(
            user_id, limit=5
        )
    ...
    def test_advanced_filter_by_rating(
        self, populated_test_db, movie_manager_with_test_db
    ):
        """Test advanced filtering by rating."""
        # Filter movies with rating >= 8.5
        movies = movie_manager_with_test_db.advanced_filter(rating_min=8.5, limit=10)

        assert len(movies) > 0
        for movie in movies:
            assert movie.rating >= 8.5

    def test_advanced_filter_by_genre(
        self, populated_test_db, movie_manager_with_test_db
    ):
        """Test advanced filtering by genre."""
        # Filter Action movies
        movies = movie_manager_with_test_db.advanced_filter(genres=["Action"], limit=10)

        assert len(movies) > 0
        for movie in movies:
            assert "Action" in movie.genres

    def test_advanced_filter_by_director(
        self, populated_test_db, movie_manager_with_test_db
    ):
        """Test advanced filtering by director."""
        # Filter Christopher Nolan movies
        movies = movie_manager_with_test_db.advanced_filter(
            directors=["Christopher Nolan"], limit=10
        )
    ...
    def test_advanced_filter_multiple_criteria(
        self, populated_test_db, movie_manager_with_test_db
    ):
        """Test advanced filtering with multiple criteria."""
        # Filter Action movies from 2000+ with rating >= 8.0
        movies = movie_manager_with_test_db.advanced_filter(
            year_min=2000, rating_min=8.0, genres=["Action"], limit=10
        )
    ...
    def test_advanced_filter_exclude_watched(
        self, populated_test_db, movie_manager_with_test_db, sample_user
    ):
        ...
        unwatched_movies = movie_manager_with_test_db.advanced_filter(
            exclude_watched=True, user_id=user_id, limit=10
        )
        ...
        watched_movies = movie_manager_with_test_db.get_watch_history(
            user_id, limit=100
        )
