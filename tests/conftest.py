...
def mock_db_manager(test_db_session):
    ...
    with patch("src.core.movie_manager.db_manager") as mock_manager1, patch(
        "src.core.filters.db_manager"
    ) as mock_manager2, patch("src.core.recommendation.db_manager") as mock_manager3:
        ...
        for mock_manager in [mock_manager1, mock_manager2, mock_manager3]:
            mock_manager.get_session.return_value.__enter__.return_value = (
                test_db_session
            )
...
def populated_test_db(test_db_session, sample_movies, sample_user):
    ...
    ratings = [
        UserRating(
            user_id=user_id, movie_id=movie1_id, rating=9, review="Excellent movie!"
        ),
        UserRating(
            user_id=user_id, movie_id=movie2_id, rating=8, review="Great storytelling."
        ),
    ]
