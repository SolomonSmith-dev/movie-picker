...
async def list_movies(
    ...
    order_by: str = Query("rating", description="Order by field (rating, year, title, runtime)"),
...
async def get_movie_of_the_day():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting movie of the day: {str(e)}")
...
async def pick_random_movie(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error picking random movie: {str(e)}")
...
async def get_recent_movies(
    ...
    try:
        ...
        valid_movies = [movie for movie in movies if movie.year and isinstance(movie.year, int)]
        return [MovieResponse.from_orm(movie) for movie in valid_movies[:limit]]
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching recent movies: {str(e)}")
...
async def get_movie_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching movie stats: {str(e)}")
