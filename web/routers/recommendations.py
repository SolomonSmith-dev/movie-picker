...
async def get_recommendations(
    user_id: int,
    limit: int = Query(
        10, ge=1, le=50, description="Number of recommendations per category"
    ),
):
...
async def get_surprise_recommendation(user_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching surprise recommendation: {str(e)}"
        )
...
async def get_similar_movies(
    ...
    movie_id: int,
    limit: int = Query(
        5, ge=1, le=20, description="Number of similar movies to return"
    ),
):
    ...
    try:
        ...
        if target_movie.genres:
            similar_movies = movie_manager.advanced_filter(
                genres=[target_movie.genres.split(",")[0].strip()], limit=limit
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching similar movies: {str(e)}"
        )
...
async def get_user_preferences(user_id: int):
    ...
    try:
        ...
        if not history:
            return {"message": "No watch history available", "preferences": {}}
        ...
        avg_rating = (
            sum(entry["rating"] for entry in rated_movies) / len(rated_movies)
            if rated_movies
            else 0
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching user preferences: {str(e)}"
        )
